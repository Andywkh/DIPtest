...
#out = cv2.VideoWriter('output3.avi', fourcc, 30.0, (1440,1080))
...
final_gtaoa = [[]]
final_calculatedaoa = [[]]
final_gtaoa_indegree = [[]]
final_calculatedaoa_indegree = [[]]
...
# Capture frame-by-frame
   ...
   # if valid_T3:
    ...
    u_gt_T1 = fx * gtaoa_tan_for_frame_T1 + u_0
    ...
    if frameNumber > 150: # do not use the first 5 sec' data
        ...
        final_gtaoa_indegree[0].append(np.arctan(gtaoa_tan_for_frame_T1) * 180 / np.pi)
        final_calculatedaoa_indegree[0].append(np.arctan(calculatedAOA_T1) * 180 / np.pi)
...
mean_absolute_error = np.zeros((1,1))
for i in range(1):
    for j in range(1):
        mean_absolute_error[i,j] = np.mean(np.abs(np.array(final_gtaoa_indegree[i]) - np.array(final_calculatedaoa_indegree[j])))
print(mean_absolute_error)
# visualize the error with a heat map
# fig, ax = plt.subplots()
# plt.xticks(ticks = np.arange(2))
# plt.yticks(ticks = np.arange(2))
# plt.xlabel('CV obj.')
# plt.ylabel('RFID tag')
# hm = ax.imshow(mean_absolute_error, cmap='Blues', interpolation = 'nearest')
# plt.colorbar(hm)

# plt.savefig('OutputPics/test2.png')
# plt.show()

fig, ax = plt.subplots()
ax.plot(final_gtaoa_indegree[0], label = 'gtaoa_#1')
# ax.plot(final_gtaoa[1], label = 'gtaoa_#2')
ax.plot(final_calculatedaoa_indegree[0], label = 'cal.aoa_#1')
