...
## for the output file of CV object tracking algorithm
## offline processing
...
class cv_dataloader:
    def __init__(self, box_filename, box_list_wanted = ['0 ', '1 ','2 ', '-1 ']):#rfid_list_wanted = ['E280 1160 6000 0209 F811 7224'] #  ['E280 1160 6000 0209 F811 7224']['E280 6894 0000 500E 73D4 548C']
        ## cv distance ##
        # [Timestamp    id	tlu	tlv	 w	h	coor_x	coor_y	coor_z] #
        self.box_ids = []
        self.box_data = []
        with open(box_filename, 'r') as f:
            ...
            for row in f_csv:
                if row[4] != 'nan ' and  row[4] != 'inf 'and row[2] in box_list_wanted:
                    ...
                    self.box_data[self.box_ids.index(row[2])].append(row)
            f.close()
        print(self.box_ids)
        ...
        for k, person in enumerate(self.box_ids):
            ...
            #self.ctime[k] = np.array([float(i[0]) + 8 * 60 * 60 for i in self.box_data[self.box_ids.index(person)] if i[-2] != 'nan '])
            self.c_x_temp = np.array([eval(i[-4]) for i in self.box_data[self.box_ids.index(person)] if i[-2] != 'nan ']) - 0
            self.c_z_temp = np.array([eval(i[-2]) for i in self.box_data[self.box_ids.index(person)] if i[-2] != 'nan ']) + 0.08
            ...
            self.cu[k] = np.array([eval(i[2]) + 0.5 * eval(i[4]) for i in self.box_data[self.box_ids.index(person)] if i[-2] != 'nan '])
