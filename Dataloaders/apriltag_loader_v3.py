...
class cv_dataloader:
    def __init__(self, box_filename, box_list_wanted = ['0 ', '1 ','2 ', '-1 ']):#rfid_list_wanted = ['E280 1160 6000 0209 F811 7224'] #  ['E280 1160 6000 0209 F811 7224']['E280 6894 0000 500E 73D4 548C']
        ...
        with open(box_filename, 'r') as f:
            f_csv = csv.reader(f)
            for row in f_csv:
                if row[4] != 'nan ' and  row[4] != 'inf 'and row[1] in box_list_wanted:
                    if row[1] not in self.box_ids:
                        self.box_ids.append(row[1])
                        self.box_data.append([])
                    self.box_data[self.box_ids.index(row[1])].append(row)
            f.close()
        print(self.box_ids)
