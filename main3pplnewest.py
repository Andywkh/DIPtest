from AOA_Calculations.sixRubberAntenna3pplnewest import *
...
cap = cv2.VideoCapture('./Latest_Test/video_raw/test2.mp4')

##### To Output Video when running #####
fourcc = cv2.VideoWriter_fourcc(*'XVID')
out = cv2.VideoWriter('final232323.avi', fourcc, 30.0, (1440,1080))
...
avg_window = 250
calculatedAOA_T1_new_hist = []
calculatedAOA_T2_new_hist = []
calculatedAOA_T3_new_hist = []
...
# Capture frame-by-frame
    ...
    calculatedAOA_T1_new_hist.append(calculatedAOA_T1)
    calculatedAOA_T2_new_hist.append(calculatedAOA_T2)
    calculatedAOA_T3_new_hist.append(calculatedAOA_T3)

    calculatedAOA_T1_avg =  np.mean(calculatedAOA_T1_new_hist[-avg_window:])
    calculatedAOA_T2_avg =  np.mean(calculatedAOA_T2_new_hist[-avg_window:])
    calculatedAOA_T3_avg =  np.mean(calculatedAOA_T3_new_hist[-avg_window:])
        #1080p
    # fx = 528.6107177
    # u_0 = 630.80725
    ...
    # ###### Draw the calculated/estimated AOA (rfid) ######
    # RFID_Tag 1 (John)    
    u_T1 = fx * math.tan(calculatedAOA_T1_avg) + u_0
    ...
    # RFID_Tag 2 (SiMin)   
    u_T2 = fx * math.tan(calculatedAOA_T2_avg) + u_0
    ...
    #RFID_Tag 4 (Deva)   
    u_T3 = fx * math.tan(calculatedAOA_T3_avg) + u_0
    ...
    if frameNumber > 150 and frameNumber < 900: # do not use the first 5 sec' data
...
plt.savefig('OutputPics/111.png')
plt.show()
...
plt.legend()
plt.savefig('OutputPics/222.png')
plt.show()
